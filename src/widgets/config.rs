    ...
    match key.code {
        KeyCode::Enter => {
            ...
            app.config_options.options.iter().for_each(|option| {
                config
                    .with_section(Some("Main".to_string()))
...
pub fn render_config(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let block = Block::bordered()
        .title_top(Line::from(" Config ").bold().centered())
        .title_bottom(instructions.centered())
        .border_set(border::THICK);

    let widths = [Constraint::Min(10), Constraint::Min(10)];

    let rows = app
        .config_options
        .options
        .iter()
        .map(|option: &ConfigOption| Row::new(vec![option.name.clone(), option.value.clone()]));
